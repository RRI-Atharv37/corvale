...
/** Start of day in the given timezone, returned as a UTC Date for storage/queries. */
...
/** End of day in the given timezone, returned as a UTC Date for storage/queries. */
...
/**
 * Converts an amount between currencies using a rate map keyed 'FROM_TO'.
 * Falls back to the inverse pair, then to a 1:1 rate when no rate is
 * configured, so multi-currency totals never throw for a missing config.
 */
export const convertAmount = (
    ...
    if (typeof rates[reverseKey] === 'number' && rates[reverseKey] !== 0) {
        return amount * (1 / rates[reverseKey])
    }
