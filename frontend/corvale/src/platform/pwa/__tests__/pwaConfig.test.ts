...
describe('PWA config (Sprint 13.8)', () => {
    ...
    it('the Background Sync tag and message constants match between sw.ts and backgroundSync.ts (no drift)', () => {
        const swSource = readSource('src/sw.ts')
        const backgroundSyncSource = readSource('src/platform/pwa/backgroundSync.ts')
        const constantsSource = readSource('src/platform/pwa/constants.ts')

        expect(swSource).toContain("from '@platform/pwa/constants'")
    ...
    it('React.lazy code-splits every dashboard page instead of statically importing all 25', () => {
        ...
        expect(appSource).toContain("lazy(() => import('@features/dashboard/HomePage'))")
        expect(appSource).toContain("lazy(() => import('@features/reports/ReportsPage'))")
        ...
        expect(appSource).toContain("import Login from '@features/auth/LoginPage'")
    })
    ...
    it('DOCS_URL is configurable via VITE_DOCS_URL instead of hardcoded', () => {
        const layoutSource = readSource('src/app/layouts/DashboardLayout.tsx')
    ...
    it('UpdatePrompt does not claim the (web) app "works offline"', () => {
        const promptSource = readSource('src/platform/pwa/UpdatePrompt.tsx')
