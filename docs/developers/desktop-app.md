...
The release is left as a **draft** - review the generated notes and attached artifacts, then publish it manually from GitHub, making sure it's marked as the latest release.

### How `/download` stays current

The web `/download` page reads its version, download links, checksums, and sizes at runtime from `GET /api/v1/desktop/release-manifest`. That endpoint proxies the GitHub Releases API server-side (`backend/services/desktopReleaseService.ts`) with a short in-memory cache and serves the last good response if GitHub is briefly unreachable. So publishing a release - and marking it latest - is all it takes for `/download` to update. No code change, no frontend redeploy, and the browser never has to reach GitHub directly (GitHub's release-asset URLs send no CORS headers, and the app's CSP stays tight).

`frontend/corvale/src/data/releaseManifest.ts` still holds a manifest, but only as the built-in fallback the page renders before the fetch resolves (and keeps showing if the backend or GitHub is unavailable, or before the first release is published). It also holds the editorial content that isn't in a release: each platform's label, system requirements, and which installer format is the one-click download. Update that fallback only when those change, or occasionally to match the current release for a faster first paint.
