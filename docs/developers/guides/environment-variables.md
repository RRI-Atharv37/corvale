...
| `MOR_VARIANTS` | Only if billing is on | - | JSON map of plan and interval to the provider's variant id. Nested (`{"plus":{"monthly":"1","annual":"2"},"pro":{"monthly":"3","annual":"4"}}`) or flat (`{"plus_monthly":"1","plus_annual":"2","pro_monthly":"3","pro_annual":"4"}`) both work. Every id must be different |
...
| `FINANCE_OPS_ENABLED` | No | `false` | Turn on the deferred-revenue bookkeeping surface (M8e): the billing sweep splits each annual `payment.succeeded` event into 12 monthly recognition buckets, and (while `ADMIN_ENABLED` is also true) the internal admin API exposes a `GET /admin/finance/recognition/summary` report, a no-PII `GET /admin/finance/recognition/export.csv` for a CA, and a `POST /admin/finance/recognition/run` on-demand trigger. While off, nothing is recognized and none of those routes exist. Independent of `BILLING_ENABLED` |
| `DEMO_ACCOUNT_EMAIL` | No | `demo@corvale.app` | Email of the shared, read-only demo account that `npm run seed:demo-account` creates and resets. Meant to be public - only change it if you want a different pair, and update the frontend's `VITE_DEMO_EMAIL` to match |
| `DEMO_ACCOUNT_PASSWORD` | No | `CorvaleDemo!2026` | Password of the demo account above. Also meant to be public; update the frontend's `VITE_DEMO_PASSWORD` to match if you change it |
...
| `VITE_DEMO_EMAIL` | No | unset (hidden) | Prefills the login form for the shared demo account when a visitor follows the landing page's "View demo" link. Leave unset to hide that link - the right default when no demo account has been seeded. Must match the backend's `DEMO_ACCOUNT_EMAIL` |
| `VITE_DEMO_PASSWORD` | No | unset (hidden) | Password half of the pair above. Must match the backend's `DEMO_ACCOUNT_PASSWORD` |
...
`SameSite=None`. See [Authentication API](../api/authentication-api.md#desktop-clients).
