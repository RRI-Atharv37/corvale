...
} from '@modules/transactions/transactionUtils'
...
const loadExistingDuplicateMap = async (
    ...
    const externalIds = [
        ...new Set(rows.map((row) => row.externalId).filter((id): id is string => !!id)),
    ]
...
const prepareImport = async (input: ImportRequestInput) => {
    const resolvedWorkspaceId = parseOptionalWorkspaceId(input.workspaceId) ?? null
    if (resolvedWorkspaceId) {
        await assertWorkspaceMembership(resolvedWorkspaceId, input.userId, 'editor')
    }

    const account = await validateAccountForTransaction(input.accountId, input.userId)
    assertAccountMatchesWorkspace(account.workspaceId, resolvedWorkspaceId)
    await validateCategoryForTransaction(input.defaultCategoryId, input.userId)

    const { importRows, rowErrors } = resolveRowsAndErrors(input.body)
    ...
    return { account, resolvedWorkspaceId, importRows, rowErrors, previewItems, itemsWithDuplicates }
}

const errorItemsFrom = (
...
export const previewImport = async (input: ImportRequestInput) => {
    const { rowErrors, itemsWithDuplicates } = await prepareImport(input)

    const items = [
        ...itemsWithDuplicates,
        ...errorItemsFrom(rowErrors, input.defaultCategoryId),
    ].sort((a, b) => a.rowIndex - b.rowIndex)

    return { items, summary: summarizePreview(items) }
}

export const commitImport = async (input: ImportRequestInput) => {
    const rowDecisions = parseImportRowDecisions(input.body.rowDecisions)

    const { account, resolvedWorkspaceId, rowErrors, previewItems, itemsWithDuplicates } =
        await prepareImport(input)
