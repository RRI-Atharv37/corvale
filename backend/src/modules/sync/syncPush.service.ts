...
/**
 * Whether an existing document a create op's client-generated `_id` collided with belongs to the
 * calling user (SEC-13, BUG-02). A workspace doc is "owned" by any member; a `userId: null` doc
 * (the master-category concept) is shared and never a conflict; otherwise the doc's `userId` must
 * match the caller.
 */
...
const applyCreateOp = async (
    ...
    if (typeof clientId === 'string' && Types.ObjectId.isValid(clientId)) {
        // SEC-55: bypass the soft-delete filter so a collision with a tombstoned row is caught here
        // as an id_conflict, not missed into a duplicate-key insert.
...
/**
 * BUG-16: wraps the entity's apply logic with an atomic compare-and-set on `updatedAt` so exactly
 * one of N racers can move the row off `current.updatedAt` (the rest come back `conflict`), and the
 * row is then guaranteed to land strictly past that value even when the wall clock did not advance
 * across the apply.
 */
...
const applyUpdateOp = async (
    ...
    const isStale =
        ...
        (baseUpdatedAt !== undefined && current.updatedAt.toISOString() !== baseUpdatedAt)
    if (isStale) {
        return {
            status: 'conflict',
            resultId: current._id.toString(),
...
const applyGenericCreate = async <T extends MinimalSyncDoc & OwnableDoc>(
    ...
    const clientId = payload._id
    if (typeof clientId === 'string' && Types.ObjectId.isValid(clientId)) {
        // SEC-55: bypass the soft-delete filter so a client id colliding with a *tombstoned* row is
        // caught here rather than slipping past `isOwnedByCaller` into a duplicate-key insert.
...
/**
 * `category` can't reuse the plain-userId-ownership fallback: master categories have `userId: null`,
 * and a sync op that targets one must be rejected with CANNOT_MODIFY_MASTER rather than crash on
 * `null.toString()`.
 */
...
const ENTITY_HANDLERS: Record<string, EntityOpHandlers> = {
    account: {
        ...
        update: (userId, payload, baseUpdatedAt) =>
            applyGenericUpdate(
                ...
                (doc) =>
                    serializeAccountDocForWire(doc.toObject() as unknown as Record<string, unknown>)
            ),
    ...
    savingsGoal: {
        create: (userId, payload) =>
            applyGenericCreate(SavingsGoal, userId, payload, () =>
                createSavingsGoalForOp(userId, payload)
            ),
        update: (userId, payload, baseUpdatedAt) =>
            applyGenericUpdate(
                SavingsGoal,
    ...
    tag: {
        create: (userId, payload) =>
            applyGenericCreate(Tag, userId, payload, () => createTagForOp(userId, payload)),
        update: (userId, payload, baseUpdatedAt) =>
            applyGenericUpdate(
                Tag,
                userId,
                payload,
                baseUpdatedAt,
                ERROR_MESSAGES.TAG.TAG_NOT_FOUND,
                true,
    ...
    recurringRule: {
        create: (userId, payload) =>
            applyGenericCreate(RecurringRule, userId, payload, () =>
                createRecurringRuleForOp(userId, payload)
            ),
        update: (userId, payload, baseUpdatedAt) =>
            applyGenericUpdate(
                RecurringRule,
...
/**
 * The unique (userId, opId) index is the mutex for BUG-10: this inserts a `pending` row *before*
 * the op is applied, so two requests racing on the same opId can't both pass a read-then-write
 * idempotency check.
 */
const claimSyncOperation = async (userId: string, op: SyncOpInput): Promise<ClaimResult> => {
    try {
...
export const applyPushBatch = async (
    ...
    if (ops.length > MAX_PUSH_OPS) {
        throw new CustomError(
            `Push payload exceeds the maximum of ${MAX_PUSH_OPS} operations`,
            413
        )
    }

    const workspaceId = parseOptionalWorkspaceId(rawWorkspaceId) ?? null
    if (workspaceId) {
