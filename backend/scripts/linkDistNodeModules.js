...
// outDir/rootDir), which puts dist/backend/server.js *outside* backend/'s own directory tree.
// Node's require() only walks up from the executing file's own path looking for node_modules
// (dist/backend -> dist -> repo root -> ...), so it can never see backend/node_modules,
// which is a sibling of dist/, not an ancestor. Without this, `node ../dist/backend/server.js`
// fails with "Cannot find module 'dotenv'" (or any other dependency) even though npm install
// succeeded.
//
